        ...
        <div className="space-y-6 py-4">
          ...
          {creator.analysis_data && Object.keys(creator.analysis_data).length > 0 && (
            <div className="space-y-4">
              ...
              {/* 정량 평가 */}
              {(creator.analysis_data as any).quantitative_scores && (
                <div className="rounded-lg border p-4">
              ...
              {/* 정성 평가 */}
              {(creator.analysis_data as any).qualitative_scores && (
                <div className="rounded-lg border p-4">
              ...
              {/* 강점 */}
              {(creator.analysis_data as any).strengths &&
                Array.isArray((creator.analysis_data as any).strengths) && (
                  <div className="rounded-lg border bg-green-50 p-4">
                    <h4 className="mb-2 font-medium text-green-900">💪 강점</h4>
                    <ul className="space-y-1">
                      {(creator.analysis_data as any).strengths.map(
              ...
              {/* 약점 */}
              {(creator.analysis_data as any).weaknesses &&
                Array.isArray((creator.analysis_data as any).weaknesses) && (
                  <div className="rounded-lg border bg-orange-50 p-4">
                    <h4 className="mb-2 font-medium text-orange-900">⚠️ 약점</h4>
                    <ul className="space-y-1">
                      {(creator.analysis_data as any).weaknesses.map(
              ...
              {/* 오디언스 분석 */}
              {(creator.analysis_data as any).audience_analysis && (
              ...
              {/* 콘텐츠 스타일 분석 */}
              {(creator.analysis_data as any).content_style_analysis && (
                <div className="rounded-lg border bg-purple-50 p-4">
              ...
              {/* 추천 제품 */}
              {(creator.analysis_data as any).recommended_products &&
                Array.isArray((creator.analysis_data as any).recommended_products) && (
                  <div className="rounded-lg border p-4">
                    <h4 className="mb-3 font-medium text-gray-900">🍜 추천 제품</h4>
                    <div className="flex flex-wrap gap-2">
                      {(creator.analysis_data as any).recommended_products.map(
            ...
            Object.keys(creator.collaboration_history).length > 0 && (
              <div className="rounded-lg border bg-indigo-50 p-4">
                ...
                <div className="space-y-3">
                  {(creator.collaboration_history as any).recommended_type && (
                    <div>
                  ...
                  {(creator.collaboration_history as any).content_suggestions &&
                    Array.isArray(
                      (creator.collaboration_history as any).content_suggestions
                    ) && (
                      <div>
                        ...
                        <ul className="space-y-1">
                          {(creator.collaboration_history as any).content_suggestions.map(
                  ...
                  {(creator.collaboration_history as any).estimated_performance && (
                    <div>
                  ...
                  {(creator.collaboration_history as any).budget_recommendation && (
                    <div>
          ...
          {creator.risk_factors && Object.keys(creator.risk_factors).length > 0 && (
            <div className="rounded-lg border border-red-200 bg-red-50 p-4">
              ...
              <div className="space-y-3">
                {(creator.risk_factors as any).level && (
                  <div>
                    <span
                      className={`inline-flex rounded-full px-3 py-1 text-sm font-medium ${
                        (creator.risk_factors as any).level === 'high'
                          ? 'bg-red-200 text-red-800'
                          : (creator.risk_factors as any).level === 'medium'
                ...
                {(creator.risk_factors as any).factors &&
                  Array.isArray((creator.risk_factors as any).factors) && (
                    <div>
                      <p className="mb-2 text-sm font-medium text-gray-700">
                        리스크 요인
                      </p>
                      <ul className="space-y-1">
                        {(creator.risk_factors as any).factors.map(
                ...
                {(creator.risk_factors as any).mitigation &&
                  Array.isArray((creator.risk_factors as any).mitigation) && (
                    <div>
                      <p className="mb-2 text-sm font-medium text-gray-700">완화 방안</p>
                      <ul className="space-y-1">
                        {(creator.risk_factors as any).mitigation.map(
